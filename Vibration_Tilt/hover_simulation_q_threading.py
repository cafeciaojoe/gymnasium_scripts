...
def vibration_worker(scf):
    """
    Worker function that runs in a separate thread.
    Continuously monitors drone orientation and adjusts motor power.
    
    Args:
        scf: SyncCrazyflie object for drone communication
    """
    global vibration_running
...
def start_vibration(scf):
    """
    Start vibration control in a separate thread.
    
    Args:
        scf: SyncCrazyflie object for drone communication
    """
    global vibration_running, vibration_thread
    
    if vibration_running:
        print("Vibration already running!")
        return
    
    vibration_running = True
    vibration_thread = threading.Thread(target=vibration_worker, args=(scf,))
    vibration_thread.daemon = True  # Thread will exit when main program exits
    vibration_thread.start()

def stop_vibration():
    """
    Stop vibration control and wait for thread to finish.
    """
    global vibration_running, vibration_thread
    
    if not vibration_running:
        print("Vibration not running!")
        return
    
    vibration_running = False
    if vibration_thread:
        vibration_thread.join(timeout=2.0)  # Wait up to 2 seconds for clean shutdown
        vibration_thread = None
...
if __name__ == '__main__':
    ...
    with SyncCrazyflie(URI, cf=Crazyflie(rw_cache='./cache')) as scf:
        ...
        # Start vibration control in background thread
        start_vibration(scf)
